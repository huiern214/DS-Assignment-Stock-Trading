...
public class BuySellService {
    ...
    public BuySellService(UserService userService, PortfolioTableOperationService portfolioTableOperationService,
            StockTableOperationService stockTableOperationService,
            OrdersTableOperationService ordersTableOperationService,
            TransactionsTableOperationService transactionsTableOperationService) throws SQLException {
    ...
    public void buyStock(String stockSymbol, double desiredPrice, int desiredQuantity, int buyerId)
            throws SQLException {
        ...
        List<Order> matchingOrders = ordersTableOperationService.getMatchingSellOrders(stockSymbol, desiredPrice,
                buyerId);
        int leftoverStockQuantity = desiredQuantity;
        int totalQuantityInMatchingOrders = 0;
        // int tempSystemStockQuantity =
        // stockTableOperationService.getStockQuantity(stockSymbol);
        // Check if the matchingOrders meets the desiredQuantity of stocks to be
        // purchased
        ...
        if ((matchingOrders != null) && (matchingOrders.size() != 0)) {
            for (Order matchingOrder : matchingOrders) {
                ...
                if (sellerQuantity >= leftoverStockQuantity) {
                    ...
                    userService.updateUserFunds(buyerId,
                            (userService.getUserFunds(buyerId)) - (leftoverStockQuantity * 100 * sellerPrice)); // 100
                                                                                                                // is
                                                                                                                // the
                                                                                                                // number
                                                                                                                // of
                    ...
                    portfolioTableOperationService.addStockToPortfolio(buyerId, stockSymbol, leftoverStockQuantity,
                            desiredPrice);

                    // Add transaction for the buyer
                    transactionsTableOperationService.insertTransaction(buyerId, stockSymbol, desiredPrice,
                            leftoverStockQuantity, "BUY");
                    // If the desired quantity still have leftover, then the leftoverStockQuantity
                    // will keep decreasing until 0 for each trade
                    ...
                    // Update buyer's funds
                    userService.updateUserFunds(buyerId,
                            (userService.getUserFunds(buyerId)) - (sellerQuantity * 100 * sellerPrice)); // 100 is the
                                                                                                         // number of
                                                                                                         // stocks per
                                                                                                         // lot

                    // Add stock to buyer's portfolio
                    portfolioTableOperationService.addStockToPortfolio(buyerId, stockSymbol, sellerQuantity,
                            desiredPrice);
            ...
            // Happens when there are no system stock available, so an Order will be placed
            // in the Orders Table to see
            ...
            // Add transaction for the buyer
            transactionsTableOperationService.insertTransaction(buyerId, stockSymbol, desiredPrice, desiredQuantity,
                    "BUY");
        ...
        // If there are Matching Orders, but after buying from Orders, there are still
        // leftovers, then it will buy from system stock
        if (leftoverStockQuantity > 0 && desiredQuantity > leftoverStockQuantity) {
            // Step 4.5: Buy stock from the system stock
            int systemStockQuantity = stockTableOperationService.getStockQuantity(stockSymbol);
            if (systemStockQuantity < leftoverStockQuantity) {
                ordersTableOperationService.insertOrder(buyerId, stockSymbol, "BUY", leftoverStockQuantity,
                        desiredPrice);
                // return;
                throw new IllegalArgumentException("The order has been placed. Please wait for the matching order.");
            }
            ...
            // Update buyer's funds
            userService.updateUserFunds(buyerId,
                    (userService.getUserFunds(buyerId)) - (desiredPrice * 100 * leftoverStockQuantity)); // 100 is the
                                                                                                         // number of
                                                                                                         // stocks per
                                                                                                         // lot
    ...
    private void executeTrade(String stockSymbol, double price, int quantity, int sellerId, double sellerPrice,
        ...
        PortfolioItem portfolioitem = portfolioTableOperationService.getEarliestMatchingPortfolioItem(sellerId,
                stockSymbol);
        ...
        // Deduct the stock quantity from the seller's available quantity in the Orders
        // table
        ...
        userService.updateUserFunds(sellerId, sellerBalance + (quantity * 100 * sellerPrice)); // 100 is the number of
                                                                                               // stocks per lot
    ...
    private void executePartialTrade(String stockSymbol, double price, int quantity, int sellerId, double sellerPrice,
            int sellerQuantity, int orderId) throws SQLException {
        // Execute a partial trade when the matched selling order has a lesser quantity
        // than the desired quantity
        // Deduct the stock quantity from the seller's portfolio in the Portfolio table
        PortfolioItem portfolioitem = portfolioTableOperationService.getEarliestMatchingPortfolioItem(sellerId,
                stockSymbol, sellerQuantity, price);
        int portfolioId = portfolioitem.getPortfolioId();
        ...
        double sellerBalance = userService.getUserFunds(sellerId);
        userService.updateUserFunds(sellerId, sellerBalance + (sellerQuantity * 100 * sellerPrice)); // 100 is the
                                                                                                     // number of stocks
                                                                                                     // per lot

        // Add the stock to the buyer's portfolio in the Portfolio table with the
        // purchased quantity
        // portfolioTableOperationService.addStockToPortfolio(sellerId, stockSymbol,
        // sellerQuantity, price);
    ...
    public void sellStock(String stockSymbol, double desiredPrice, int desiredQuantity, int sellerId)
            throws SQLException {
        ...
        // Step 4: Check if there is a matching order in the Order table
        List<Order> matchingOrders = ordersTableOperationService.getMatchingBuyOrders(stockSymbol, desiredPrice,
                sellerId);
        ...
        if ((matchingOrders != null) && (matchingOrders.size() != 0)) {
            for (Order matchingOrder : matchingOrders) {
                ...
                // If buyer's Order isn't 0, then it will keep looping until either the seller
                // sold enough, or the buyer's Order is 0, then only move to the next Order.
                while (buyerQuantity != 0) {
                    // get the earliest portfolio quantity in the seller's portfolio
                    PortfolioItem firstPortfolio = portfolioTableOperationService
                            .getEarliestMatchingPortfolioItem(sellerId, stockSymbol);
                    ...
                    if (buyerQuantity >= leftoverStockQuantity) {
                        int tradeQuantity = (firstPortfolioQuantity < leftoverStockQuantity) ? firstPortfolioQuantity
                                : leftoverStockQuantity;
                        // Affect the buyer's side of the trade
                        executeSellTrade(stockSymbol, desiredPrice, tradeQuantity, buyerId, buyerPrice, buyerQuantity,
                                orderId);

                        // Affect the seller's side of the trade
                        // Update seller's funds
                        userService.updateUserFunds(sellerId,
                                (userService.getUserFunds(sellerId)) + (tradeQuantity * 100 * desiredPrice)); // 100 is
                                                                                                              // the
                                                                                                              // number
                                                                                                              // of
                                                                                                              // stocks
                        ...
                        PortfolioItem portfolioitem = portfolioTableOperationService
                                .getEarliestMatchingPortfolioItem(sellerId, stockSymbol);
                        int portfolioId = portfolioitem.getPortfolioId();
                        portfolioTableOperationService.updateStockQuantity(portfolioId, stockSymbol,
                                firstPortfolioQuantity - tradeQuantity);
                        ...
                        transactionsTableOperationService.insertTransaction(sellerId, stockSymbol, desiredPrice,
                                tradeQuantity, "SELL");
                        ...
                        // Since buyerQuantity will be bigger or equal than the quantity in the
                        // portfolio, so there are still some quantity left to be bought in the Order
                        buyerQuantity = buyerQuantity - tradeQuantity;

                    } else {
                        // This should happen when the buyer's Order is less than or equal the
                        // firstPortfolioQuantity
                        // as well as the desired Quantity to sell is higher than or equal to the
                        // buyer's Order
                        // Step 5.2: Execute a partial trade
                        int tradeQuantity = (firstPortfolioQuantity >= leftoverStockQuantity) ? buyerQuantity
                                : firstPortfolioQuantity;
        ...
        // If there are still leftover quantity to be sold, then the remaining will be
        // placed as an Order
    ...
    // Only used in selling stock to update the buyer's side of the trade when the
    // quantity in the buyer's order is bigger than the seller's first portfolio
    // amount
    private void executeSellTrade(String stockSymbol, double tradePrice, int tradeQuantity, int buyerId,
            double buyerPrice,
    ...
    // Only used in selling stock to update the buyer's side of the trade when the
    // quantity in the buyer's order is smaller than the seller's first portfolio
    // amount
    private void executePartialSellTrade(String stockSymbol, double tradePrice, int tradeQuantity, int buyerId,
            double buyerPrice,
            int buyerQuantity, int orderId) throws SQLException {
        // Calculate the total trade value
        double totalTradeValue = tradePrice * tradeQuantity * 100; // 100 is the number of stocks per lot
